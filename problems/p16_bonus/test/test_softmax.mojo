...
from op import run_gpu_softmax, softmax_cpu_kernel
...
def test_softmax():
    with DeviceContext() as ctx:
        out = ctx.create_buffer[DType.float32](SIZE).fill(0)
        inp = ctx.create_buffer[DType.float32](SIZE).fill(0)
        # for CPU testing
        expected = ctx.create_host_buffer[DType.float32](SIZE).fill(0)
        ...
        # Run GPU kernels
        run_gpu_softmax[layout, SIZE, dtype](output_tensor, input_tensor, ctx)
